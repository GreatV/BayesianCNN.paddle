...
class TestModelForwardpass:
    ...
    def test_cpu_bayesian(self, model):
        ...
        batch = paddle.randn(shape=(batch_size, 3, 32, 32))
        net = model(10, 3)
        out = net(batch)
...
class TestBayesianLayers:
    def test_flatten(self):
        batch_size = np.random.randint(1, 256)
        batch = paddle.randn(shape=(batch_size, 64, 4, 4))
    ...
    def test_conv(self):
        batch_size = np.random.randint(1, 256)
        batch = paddle.randn(shape=(batch_size, 16, 24, 24))
    ...
    def test_linear(self):
        batch_size = np.random.randint(1, 256)
        batch = paddle.randn(shape=(batch_size, 128))
