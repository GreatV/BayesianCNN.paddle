...
def _get_layer_wise_mean_var_per_task(mean_var_path):
    ...
    for i in range(len(files)):
        ...
        mean, var = np.vstack(mean), np.vstack(var)
        data["mean"] = mean
        data["var"] = var
...
def fit_to_gmm(num_tasks, layer_name, data_type, all_tasks):
    data = np.vstack(
        [all_tasks[f"task-{i}"][layer_name][data_type] for i in range(1, num_tasks + 1)]
    )
    data = paddle.to_tensor(data=data).astype(dtype="float32")
    model = gmm.GaussianMixture(
        n_components=num_tasks, n_features=np.prod(data.shape[1:])
    )
    data = data[paddle.randperm(n=data.shape[0])]
    model.fit(data)
...
def main():
    ...
    net_task1, net_task2 = utils_mixture.get_splitmnist_models(
        num_tasks, True, weights_dir
    )
    net_task1
    net_task2
    print(
        "Task-1 Accuracy:",
        feedforward_and_save_mean_var(net_task1, train_loader_task1, task_no=1),
    )
    print(
        "Task-2 Accuracy:",
        feedforward_and_save_mean_var(net_task2, train_loader_task2, task_no=2),
    )
