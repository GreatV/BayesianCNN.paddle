...
class BBB3Conv3FC(ModuleWrapper):
    ...
    def __init__(
        ...
        self.act1 = self.act()
        self.pool1 = paddle.nn.MaxPool2D(kernel_size=3, stride=2)
        self.conv2 = BBBConv2d(32, 64, 5, padding=2, bias=True, priors=self.priors)
        self.act2 = self.act()
        self.pool2 = paddle.nn.MaxPool2D(kernel_size=3, stride=2)
        self.conv3 = BBBConv2d(64, 128, 5, padding=1, bias=True, priors=self.priors)
        self.act3 = self.act()
        self.pool3 = paddle.nn.MaxPool2D(kernel_size=3, stride=2)
